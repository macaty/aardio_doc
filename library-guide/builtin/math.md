...
aardio 程序在启动时，主线程会以 time.tick 获得的系统启动毫秒数作为参数调用一�?math.randomize。在创建新的线程时也会自动调�?math.randomize，但不会使用系统启动毫秒数作为参数，而是自动生成一个安全的随机数作为随机数种子�?
 参考： [随机数](https://www.aardio.com/zh-cn/doc/library-guide/builtin/string/rand.html)
...
获取随机�?
 参考： [随机数](https://www.aardio.com/zh-cn/doc/library-guide/builtin/string/rand.html)math.random(5, 99) //返回\[5,99\]之间的随机数
