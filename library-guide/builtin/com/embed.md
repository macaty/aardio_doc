...
但是通常我们需要容器对象，这是因为�?
- 我们通常需要对 COM 控件做进一步的封装以增强或简�?COM 控件的功能�?COM 控件自己已经有了锁定的元表不能被替换，所以最好的方式是用一个容器对象去封装 COM 对象。参考： [元表 \- 代理表](https://www.aardio.com/zh-cn/doc/language-reference/datatype/table/meta.html#proxy)
...
createEmbedEx 函数返回的容器对象已自动添加元表并成为了 COM 对象�?[代理表](https://www.aardio.com/zh-cn/doc/language-reference/datatype/table/meta.html#proxy)，可通过容器对象的成员代理访�?COM 对象成员。也可以通过指定容器对象的成员函数响�?COM 对象事件�?
