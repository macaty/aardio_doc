...
# 输入法与键盘状态检�?
aardio 标准�?[key.ime](https://www.aardio.com/zh-cn/doc/library-reference/key/ime.html) 提供输入法相关函数�?
key.ime.state 函数则用于检测输入法与键盘状态，标准�?key.ime.stateBar 用于创建显示输入法状态的工具界面。基�?key.ime.stateBar 实现的开源软�?[ImTip](https://imtip.aardio.com/) 目前已经成为了流行的桌面软件�?
## 一、输入法状态检测原理与规则
