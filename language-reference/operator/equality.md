...
对于 table、cdata、bufffer 等传引用类型，当引用同一个对象时相等。否则检查参考比较的两对象是否存在相同的 `_eq` [元方法](https://www.aardio.com/zh-cn/doc/language-reference/datatype/table/meta.html) ，如果存在就调用 `_eq` 元方法判断是否相等。如果操作数不是同一个对象且没有相同�?`_eq` 元方法则不相等�?
